...
const sendEmail = (to, subject, html) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const mailOptions = {
            from: `"${config_1.default.email.from_name}" <${config_1.default.email.from_email}>`,
            to,
            subject,
            html,
        };
...
const sendWelcomeEmail = (email, name, role) => __awaiter(void 0, void 0, void 0, function* () {
...
const sendAgentApprovalEmail = (email, name) => __awaiter(void 0, void 0, void 0, function* () {
...
const sendTransactionEmail = (email, name, transactionDetails) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    <head>
      <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
        .container { max-width: 600px; margin: 0 auto; padding: 20px; }
        .header { background: ${isDebit ? '#E74C3C' : '#27AE60'}; color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
        .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
        .transaction-box { background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid ${isDebit ? '#E74C3C' : '#27AE60'}; }
        .amount { font-size: 32px; font-weight: bold; color: ${isDebit ? '#E74C3C' : '#27AE60'}; margin: 10px 0; }
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1>${isDebit ? '💸' : '💰'} Transaction ${transactionDetails.type.charAt(0).toUpperCase() +
        transactionDetails.type.slice(1)}</h1>
        </div>
        <div class="content">
          <h2>Hello ${name},</h2>
          ...
          <div class="transaction-box">
            <div class="amount">${isDebit ? '-' : '+'} ${transactionDetails.amount} Taka</div>
        ...
        : ''}
              <div class="details-row"><span>New Balance:</span><span><strong>${transactionDetails.newBalance} Taka</strong></span></div>
              <div class="details-row"><span>Transaction ID:</span><span>${transactionDetails.transactionId}</span></div>
              <div class="details-row"><span>Date & Time:</span><span>${transactionDetails.date}</span></div>
            </div>
          </div>
...
const sendAccountStatusEmail = (email, name, status) => __awaiter(void 0, void 0, void 0, function* () {
    const subject = `Account ${status === 'blocked' ? 'Suspended' : 'Reactivated'} - AmarCash`;
    const html = `
    <!DOCTYPE html>
    <html>
    <head>
      <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
        .container { max-width: 600px; margin: 0 auto; padding: 20px; }
        .header { background: ${status === 'blocked' ? '#E74C3C' : '#27AE60'}; color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
        .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
        .alert { background: ${status === 'blocked' ? '#FFF3CD' : '#D4EDDA'}; border-left: 4px solid ${status === 'blocked' ? '#E74C3C' : '#27AE60'}; padding: 15px; margin: 20px 0; border-radius: 4px; }
        .footer { text-align: center; margin-top: 30px; color: #666; font-size: 12px; }
      </style>
    </head>
    <body>
      <div class="container">
        <div class="header">
          <h1>${status === 'blocked' ? '⚠️' : '✅'} Account ${status === 'blocked' ? 'Suspended' : 'Reactivated'}</h1>
        </div>
        <div class="content">
          <h2>Hello ${name},</h2>
...
const sendPinChangeEmail = (email, name) => __awaiter(void 0, void 0, void 0, function* () {
