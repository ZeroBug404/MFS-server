...
const getUserDashboardStats = async (userId: string) => {
  ...
  const allTransactions: any[] = await Transaction.find({
  ...
  const monthlySpending = allTransactions
    .filter(
      t => new Date(t.createdAt) >= thisMonth && t.from.toString() === userId
    )
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const amount = allTransactions
      .filter(
        t =>
          new Date(t.createdAt) >= startTime && new Date(t.createdAt) < endTime
      )
...
const getAgentDashboardStats = async (agentId: string) => {
  ...
  const agent = await User.findById(agentId).select('income balance')
  ...
  // Commission earned
  const commissionEarned = agent?.income || 0
  ...
  // Recent transactions
  const recentTransactions = allTransactions.slice(0, 10).map((t: any) => {
    const isSender = t.from?._id?.toString() === agentId
    return {
      id: t._id,
      type: isSender ? 'send' : 'receive',
      amount: t.amount,
...
const getAdminDashboardStats = async () => {
  ...
  const systemBalance = await User.aggregate([
    { $group: { _id: null, total: { $sum: '$balance' } } },
  ])

  const currentRevenue = systemBalance[0]?.total || 0
  ...
  const lastMonthRevenue = allTransactions
    .filter(
      t => new Date(t.createdAt) >= lastMonth && new Date(t.createdAt) < today
    )
  ...
  const activeUserRatio =
    totalRegisteredUsers > 0 ? activeUsers / totalRegisteredUsers : 1
...
const getSystemMetrics = async (timeRange = '7d') => {
  ...
  const previousTransactions: any = await Transaction.find({
    createdAt: { $gte: previousPeriodStart, $lt: startDate },
  }).lean()
  const previousVolume = previousTransactions.reduce(
    (sum: number, t: any) => sum + t.amount,
    0
  )
  const volumeGrowth =
  ...
  const activeUserIds = new Set(
    allTransactions
      .map((t: any) =>
        typeof t.from === 'string' ? t.from : t.from?._id?.toString()
      )
      .filter(Boolean)
  )
  ...
  const revenue = allTransactions.reduce(
    (sum: number, t: any) => sum + (t.adminEarnings || 0),
    0
  )
  ...
  if (isHourly) {
    ...
    for (let i = 23; i >= 0; i--) {
      ...
      const hourTransactions = allTransactions.filter(
        (t: any) =>
          new Date(t.createdAt) >= hourStart && new Date(t.createdAt) < hourEnd
      )
  ...
  const sendMoneyRevenue = allTransactions
    ...
    .reduce((sum: number, t: any) => sum + (t.adminEarnings || 0), 0)
  ...
  // Agent Performance
  const agentTransactions = allTransactions.filter((t: any) => {
    const fromRole =
      typeof t.from === 'object' && t.from !== null && 'role' in t.from
        ? t.from.role
        : null
    const toRole =
      typeof t.to === 'object' && t.to !== null && 'role' in t.to
        ? t.to.role
        : null
  ...
  for (const transaction of agentTransactions) {
    ...
    if (
      ...
      agentId =
        typeof t.to._id === 'string' ? t.to._id : t.to._id?.toString() || null
...
export const TransactionStatisticsService = {
